 ...
 * does not alias {@code o1} (the allocation site passed to the first call). Under 1-object
 * sensitivity the two receiver objects are distinguished, so {@code v2} should only point to {@code o2}.
 */
public class QILIN1O_v2_o1_ContextPointsToTest {
    ...
    @Test
    public void testQilin1O_v2_o1_noAlias() {
        assertEquals(
                "Qilin 1-object-sensitive PTA should report NO_ALIAS for v2/o1 (v2 should only point to o2).",
                AliasTransformer.Result.PTA_NO_EVIDENCE_OF_ALIAS,
                new Driver().runAnalysis(config("v2", "o1").setCallGraph(CallGraphAlgorithm.QILIN_1O)));
    }
}
