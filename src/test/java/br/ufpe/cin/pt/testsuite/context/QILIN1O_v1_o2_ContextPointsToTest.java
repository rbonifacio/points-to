...
/**
 * Qilin 1-object-sensitive PTA on {@code br.ufpe.cin.pt.samples.context.Main#main(String[])}.
 *
 * Verifies that {@code v1} (return value of the first {@code B.foo} call, which receives {@code o1})
 * does not alias {@code o2} (the allocation site passed to the second call). Under 1-object
 * sensitivity the two receiver objects are distinguished, so {@code v1} should only point to {@code o1}.
 */
public class QILIN1O_v1_o2_ContextPointsToTest {
    ...
    @Test
    public void testQilin1O_v1_o2_noAlias() {
        assertEquals(
                "Qilin 1-object-sensitive PTA should report NO_ALIAS for v1/o2 (v1 should only point to o1).",
                AliasTransformer.Result.PTA_NO_EVIDENCE_OF_ALIAS,
                new Driver().runAnalysis(config("v1", "o2").setCallGraph(CallGraphAlgorithm.QILIN_1O)));
    }
}
