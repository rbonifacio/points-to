 ...
 * does not alias {@code o1} when the two call sites to {@code B.foo} are separated by 2-callsite
 * sensitivity.
 */
public class QILIN2C_v2_o1_ContextPointsToTest {
    ...
    @Test
    public void testQilin2C_v2_o1_noAlias() {
        assertEquals(
                "Qilin 2-callsite-sensitive PTA should report NO_ALIAS for v2/o1 (v2 should only point to o2).",
                AliasTransformer.Result.PTA_NO_EVIDENCE_OF_ALIAS,
                new Driver().runAnalysis(config("v2", "o1").setCallGraph(CallGraphAlgorithm.QILIN_2C)));
    }
}
