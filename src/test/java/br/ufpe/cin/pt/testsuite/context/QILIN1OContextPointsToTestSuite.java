 ...
 *
 * The two calls to {@code B.foo} are distinguished by their receiver objects ({@code b1} vs
 * {@code b2}), so {@code v1} and {@code v2} should not alias under 1-object sensitivity.
 */
public class QILIN1OContextPointsToTestSuite {
    ...
    @Test
    public void testQilin1O_v1_v2_noAlias() {
        assertEquals(
                "Qilin 1-object-sensitive PTA should report NO_ALIAS for v1/v2 (separate receiver objects).",
                AliasTransformer.Result.PTA_NO_EVIDENCE_OF_ALIAS,
                new Driver().runAnalysis(config("v1", "v2").setCallGraph(CallGraphAlgorithm.QILIN_1O)));
    }
}
