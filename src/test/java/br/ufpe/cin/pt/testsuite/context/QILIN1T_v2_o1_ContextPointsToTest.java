 ...
 * may alias {@code o1} when the two calls to {@code B.foo} are merged by 1-type sensitivity (both
 * receivers have the same static type {@code B}).
 */
public class QILIN1T_v2_o1_ContextPointsToTest {
    ...
    @Test
    public void testQilin1T_v2_o1_mayAlias() {
        assertEquals(
                "Qilin 1-type-sensitive PTA should report MAY_ALIAS for v2/o1 when calls to B.foo are merged by type context.",
                AliasTransformer.Result.PTA_SUGGESTS_ALIAS,
                new Driver().runAnalysis(config("v2", "o1").setCallGraph(CallGraphAlgorithm.QILIN_1T)));
    }
}
