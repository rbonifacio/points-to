 ...
 * {@code B}, so 2-type sensitivity assigns them the same type context and merges the two calls
 * to {@code B.foo}. As a result {@code v1} and {@code v2} may alias.
 */
public class QILIN2TContextPointsToTestSuite {
    ...
    @Test
    public void testQilin2T_v1_v2_mayAlias() {
        assertEquals(
                "Qilin 2-type-sensitive PTA should report MAY_ALIAS for v1/v2 (same receiver type B merges both calls).",
                AliasTransformer.Result.PTA_SUGGESTS_ALIAS,
                new Driver().runAnalysis(config("v1", "v2").setCallGraph(CallGraphAlgorithm.QILIN_2T)));
    }
}
