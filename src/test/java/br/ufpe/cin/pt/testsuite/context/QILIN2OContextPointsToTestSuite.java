 ...
 *
 * With 2-object sensitivity the two calls to {@code B.foo} are kept separate, so
 * {@code v1} and {@code v2} should not alias.
 */
public class QILIN2OContextPointsToTestSuite {
    ...
    @Test
    public void testQilin2O_v1_v2_noAlias() {
        assertEquals(
                "Qilin 2-object-sensitive PTA should report NO_ALIAS for v1/v2 (separate call contexts).",
                AliasTransformer.Result.PTA_NO_EVIDENCE_OF_ALIAS,
                new Driver().runAnalysis(config("v1", "v2").setCallGraph(CallGraphAlgorithm.QILIN_2O)));
    }
}
