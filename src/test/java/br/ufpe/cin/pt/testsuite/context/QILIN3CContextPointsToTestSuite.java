...
/**
 * Qilin 3-callsite-sensitive PTA on {@code br.ufpe.cin.pt.samples.context.Main#main(String[])}.
 *
 * With 3-callsite sensitivity the two calls to {@code B.foo} are kept separate, so
 * {@code v1} and {@code v2} should not alias.
 */
public class QILIN3CContextPointsToTestSuite {
    ...
    @Test
    public void testQilin3C_v1_v2_noAlias() {
        assertEquals(
                "Qilin 3-callsite-sensitive PTA should report NO_ALIAS for v1/v2 (separate call contexts).",
                AliasTransformer.Result.PTA_NO_EVIDENCE_OF_ALIAS,
                new Driver().runAnalysis(config("v1", "v2").setCallGraph(CallGraphAlgorithm.QILIN_3C)));
    }
}
