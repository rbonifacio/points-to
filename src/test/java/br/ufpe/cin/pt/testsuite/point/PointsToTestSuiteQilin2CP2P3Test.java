...
/**
 * Qilin 2-callsite-sensitive PTA for point2 vs point3 (alias: point3 = point2).
 * Runs in a separate JVM so Qilin/Soot static state does not affect other Qilin tests.
 */
public class PointsToTestSuiteQilin2CP2P3Test {
    ...
    @Test
    public void testPointsToQilin2CP2P3() {
        assertEquals(
                "Qilin 2-callsite-sensitive PTA should report MAY_ALIAS for point2/point3, because point3 is assigned from point2 (point3 = point2), so they may point to the same object. "
                + "This test confirms that Qilin 2C behaves as expected on this scenario.",
                AliasTransformer.Result.PTA_SUGGESTS_ALIAS,
                new Driver().runAnalysis(config.setCallGraph(CallGraphAlgorithm.QILIN_2C)));
    }
}
