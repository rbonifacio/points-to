...
/**
 * Qilin 2-hybrid-type-sensitive PTA on {@code br.ufpe.cin.pt.samples.context.Main#main(String[])}.
 *
 * In the context scenario both receivers ({@code b1} and {@code b2}) are declared as type
 * {@code B}. The type-sensitive component of hybrid-type sensitivity assigns them the same
 * type context and merges the two calls to {@code B.foo}, so {@code v1} and {@code v2} may alias.
 */
public class QILIN2HTContextPointsToTestSuite {
    ...
    @Test
    public void testQilin2HT_v1_v2_mayAlias() {
        assertEquals(
                "Qilin 2-hybrid-type-sensitive PTA should report MAY_ALIAS for v1/v2 (same receiver type B merges both calls).",
                AliasTransformer.Result.PTA_SUGGESTS_ALIAS,
                new Driver().runAnalysis(config("v1", "v2").setCallGraph(CallGraphAlgorithm.QILIN_2HT)));
    }
}
