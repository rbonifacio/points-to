 ...
 * </pre>
 *
 * Context-insensitive PTA may merge the two {@code B.foo} calls and report that {@code v1} and
 * {@code v2} both point to {@code o1} and {@code o2}.
 */
public class QILINInsensContextPointsToTestSuite {
    ...
    @Test
    public void testQilinInsens_v1_v2_mayAlias() {
        assertEquals(
                "Qilin context-insensitive PTA should report MAY_ALIAS for v1/v2 (merged call contexts).",
                AliasTransformer.Result.PTA_SUGGESTS_ALIAS,
                new Driver().runAnalysis(config("v1", "v2").setCallGraph(CallGraphAlgorithm.QILIN_INSENS)));
    }
