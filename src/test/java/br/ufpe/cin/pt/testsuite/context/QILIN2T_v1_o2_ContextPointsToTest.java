 ...
 * may alias {@code o2} when the two calls to {@code B.foo} are merged by 2-type sensitivity (both
 * receivers have the same static type {@code B}).
 */
public class QILIN2T_v1_o2_ContextPointsToTest {
    ...
    @Test
    public void testQilin2T_v1_o2_mayAlias() {
        assertEquals(
                "Qilin 2-type-sensitive PTA should report MAY_ALIAS for v1/o2 when calls to B.foo are merged by type context.",
                AliasTransformer.Result.PTA_SUGGESTS_ALIAS,
                new Driver().runAnalysis(config("v1", "o2").setCallGraph(CallGraphAlgorithm.QILIN_2T)));
    }
}
